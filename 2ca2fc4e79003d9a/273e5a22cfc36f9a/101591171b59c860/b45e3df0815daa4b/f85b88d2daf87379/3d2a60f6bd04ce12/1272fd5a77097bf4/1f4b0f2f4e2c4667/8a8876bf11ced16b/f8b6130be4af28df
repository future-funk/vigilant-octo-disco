...
import { US_DIV_CREATE_DEAL_STEPS, divZodSchema } from '../config'
...
const useConfig: UseConfig = () => {
  return {
    page: {
      path: `/${
        DEAL_REQ_TYPE_BY_URL_MAP[DEAL_REQ_TYPE.DIVESTMENT as DealReqType]
      }`,
      modules: createModules({
        defaultUiState: {
          ...DEFAULT_STATE,
          requestType: DEAL_REQ_TYPE.DIVESTMENT as DealReqType,
        },
      }),
      slots: {
        wrapper: (
          <DawForm
            defaultValues={{
              ...DEFAULT_FORM_VALUES,
              requestType: DEAL_REQ_TYPE.DIVESTMENT,
            }}
        ...
        main: (
          <LayoutBody
            main={{
              ...
              right: (
                <Box sx={{ flexGrow: 1 }}>
                  <DealContent steps={US_DIV_CREATE_DEAL_STEPS} />
                </Box>
              ),
            }}
          />
