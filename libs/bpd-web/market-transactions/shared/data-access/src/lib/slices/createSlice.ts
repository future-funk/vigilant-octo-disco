...
export const MARKET_TRANSACTIONS_SLICE_KEY = 'marketTransactions'
...
>(
  ...
  const getRootState = (state: { MarketTransactionsSlice: T }) =>
    state[MARKET_TRANSACTIONS_SLICE_KEY] as T
  ...
  const slice = createBaseSlice<T, R & typeof baseReducers>({
    sliceName: MARKET_TRANSACTIONS_SLICE_KEY,
    defaultState,
    reducers,
  })
